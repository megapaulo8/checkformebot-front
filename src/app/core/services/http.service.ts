...
import { cacheExpiresDelay, realTimeUpdateDelay, API } from './../../../environments/environment';
...
export class Http {
    ...
    if (!cache) {
      ...
      if (!this.repeatSubjects[url]) {
        ...
        this.repeatObservables[url] = new Observable<T>(() =>
          this.get<T>(url)
            .subscribe((values) => this.repeatSubjects[url].next(values))
        );
        this.repeatObservables[url].subscribe();

        setTimeout(() => this.repeatSubjects[url] = undefined, cacheExpiresDelay);
      }
  ...
  public getRepeated<T>(url: string, update?: boolean): Observable<T> {
    if (this.newRequisitionUrl[url] === undefined) this.newRequisitionUrl[url] = true;
    if (this.stopAutoUpdateValue[url] === undefined) this.stopAutoUpdateValue[url] = false;
    ...
    if (!this.repeatObservables[url] || update || this.newRequisitionUrl[url]) {
      ...
      this.repeatObservables[url] = new Observable(() => {
        if (this.repeatSubscription[url] && !this.repeatSubscription[url].closed)
          this.repeatSubscription[url].unsubscribe();

        this.repeatSubscription[url] = this.get<T[]>(url)
          .subscribe((values) => {
            this.repeatSubjects[url].next(values);

            if (this.stopAutoUpdateValue[url] === false)
              setTimeout(() => this.getRepeated(url, true), realTimeUpdateDelay);
            else {
  ...
  public execute<TType>(method: any, route: string, data?: any): Observable<TType | boolean> {
